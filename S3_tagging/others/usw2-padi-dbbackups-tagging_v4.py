...
list_of_folders = ["Prod/PAM-M2SQL-04/", "Prod/PAM-M2SQLR3$M2SQLAG1/", "Prod/PAM-MACOLA-04/"]
...
def add_tag(key, tag_key, tag_value):
    if not is_other_tag(BUCKET_CONSTANT, key, tag_key):
...
def PAM_M2SQLR3_M2SQLAG1_tagging(objname, date_key, content):
    ...
    if objname == 'M2':
        ...
        if date_key in first_days_of_all_months_in_year and time_between_last_modified.days > 31:
        ...
        elif date_key not in first_days_of_all_months_in_year and time_between_last_modified.days > 62:
            add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_M2_EXPIRATION', 'true')
            tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_M2_EXPIRATION'})
    else: 
        # 1st backup of each month moved to glacier after 28 days and expired after 122 days (4 months)
        if date_key in first_days_of_all_months_in_year and time_between_last_modified.days > 28:
