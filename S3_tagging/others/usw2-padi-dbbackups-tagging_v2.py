...
def get_first_sundays_of_months_all_year():
    ...
    for year_iter in range(year-2,year):
        for i in range(1,10): 
...
def PAM_M2SQL_04_tagging(objname, date_key, content):
    ...
    if time_between_last_modified.days > 28:
        add_tag(content['Key'], 'PAM_M2SQL_04_EXPIRATION', 'true')
        tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQL_04_EXPIRATION'})
...
def PAM_M2SQLR3_M2SQLAG1_tagging(objname, date_key, content):
    ...
    first_sunday_day_of_all_months_prev_years = get_first_sundays_of_months_all_year()
    ...
    if objname == 'M2':
        ...
        if ( date_key in first_days_of_all_months_in_year or date_key in first_sunday_day_of_all_months_prev_years ) and time_between_last_modified.days > 31:
            add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_M2_GLACIER', 'true')
            tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_M2_GLACIER'})
            if date_key not in first_day_of_jan_all_year and time_between_last_modified.days > 740 :
                add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_M2_EXPIRATION', 'true')
                tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_M2_EXPIRATION'})
        # expire all other Sunday(s) weekly backups after 2 months (62 days)
        elif ( date_key not in first_days_of_all_months_in_year or date_key not in first_sunday_day_of_all_months_prev_years ) and time_between_last_modified.days > 62:
            add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_M2_EXPIRATION', 'true')
            tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_M2_EXPIRATION'})
    else: 
        # 1st backup of each month moved to glacier after 28 days and expired after 122 days (4 months)
        if ( date_key in first_days_of_all_months_in_year or date_key in first_sunday_day_of_all_months_prev_years ) and time_between_last_modified.days > 28:
            add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_GLACIER', 'true')
            tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_GLACIER'})
            if date_key not in first_day_of_jan_all_year and time_between_last_modified.days > 122 :
                add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_EXPIRATION', 'true')
                tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_EXPIRATION'})
        # expire all other Sunday(s) weekly backups after 2 months (62 days)
        elif ( date_key not in first_days_of_all_months_in_year or date_key not in first_sunday_day_of_all_months_prev_years ) and time_between_last_modified.days > 62:
            add_tag(content['Key'], 'PAM_M2SQLR3_M2SQLAG1_EXPIRATION', 'true')
            tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_M2SQLR3_M2SQLAG1_EXPIRATION'})
...
def PAM_MACOLA_04_tagging(objname, date_key, content):
    ...
    if date_key in last_days_of_all_months_in_year and time_between_last_modified.days > 31:
        ...
        tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_MACOLA_04_DEEP_ARCHIVE'})
    # expire all other Sunday(s) weekly backups after 2 months (62 days)
    elif date_key not in last_days_of_all_months_in_year and time_between_last_modified.days > 62:
        add_tag(content['Key'], 'PAM_MACOLA_04_EXPIRATION', 'true')
        tag_added_objects_list.append({'Key':content['Key'], 'Tag':'PAM_MACOLA_04_EXPIRATION'})
