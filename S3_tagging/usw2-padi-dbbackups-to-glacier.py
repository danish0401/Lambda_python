...
def check_add_tag(content):
    ...
    if date_key != False:
        ...
        if dbname in list_of_database_names and '/FULL/' in content['Key'] and content['StorageClass'] == 'STANDARD':
            ...
            if (time_between_last_modified.days < 14):
                add_tag(content['Key'], 'S3_2_week', 'true')
                tag_added_objects_list.append({'Key':content['Key'], 'Tag':'S3_2_week_test'})
            ...
            elif date_key in last_sundays_of_month_in_all_years and time_between_last_modified.days > 14:
                add_tag(content['Key'], 'Glacier_last_sunday_m', 'true')
                tag_added_objects_list.append({'Key':content['Key'], 'Tag':'Glacier_last_sunday_m'})
